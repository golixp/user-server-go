...
graph:
	...
	@cp -f cmd/user_server_go/main.go .
	go-callvis -skipbrowser -format=svg -nostd -file=user_server_go user-server-go
	@rm -f main.go user_server_go.gv
...
.PHONY: build
# Build user_server_go for linux amd64 binary
build:
	@echo "building 'user_server_go', linux binary file will output to 'cmd/user_server_go'"
	@cd cmd/user_server_go && CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build
